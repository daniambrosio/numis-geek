...
def main(workspace_name: str, period_end: date) -> None:
    db = SessionLocal()
    try:
        ...
        if not ws:
            ...
            sys.exit(2)
        result = create_snapshot(db, workspace_id=ws.id, period_end=period_end)
        db.commit()
    except Exception:
        db.rollback()
        raise
...
if __name__ == "__main__":
    ...
    main(args.workspace, date.fromisoformat(args.period))
