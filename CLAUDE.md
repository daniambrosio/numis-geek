...
- **Rotas canônicas em inglês, labels de UI em português** (decidido 2026-08-09, vale pro app todo). Rotas PT legadas viram `<Navigate replace>` pra EN — nunca o contrário. Ex.: `/credit-cards` (label "Cartões"), `/transactions` (label "Movimentações"), `/financial-institutions` (label "Instituições"). Rotas novas nascem só em EN.
...
Every spec in `specs/*.md` must declare its status using exactly one of these three values:

- **`📝 Draft`** — being written or waiting on interview/decision. Not implementable yet.
- **`🚧 In Progress`** — actively being built. Code exists but not all deliverables landed.
- **`✅ Done`** — all deliverables shipped and tested. If superseded by a later spec, note it inline: `✅ Done (superseded by spec 32)`.

Do NOT introduce variants like `Implementing`, `Ready`, `Planning`, `Planned`, `Design done`, `Parcial`, `Done V1`, `Completed`. When a spec becomes obsolete without being finished, mark it `✅ Done (deprecated — reason)` rather than leaving it In Progress forever.
